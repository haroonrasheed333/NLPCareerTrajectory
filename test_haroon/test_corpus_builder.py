...
class TestCorpusBuilder:
    ...
    def setUp(self):
        ...
        Remove all the files generated during previous test runs and create necessary directories
        """
        self.remove_files_created_during_previous_tests()
    ...
    def test_should_extract_resume_content_from_xml_files_and_save_as_plaintext_files(self):
        filenames_before = next(os.walk(self.plaintext_directory))[2]
        assert_equals(0, len(filenames_before))

        prepare_data(self.paths)

        filenames_after = next(os.walk(self.plaintext_directory))[2]
        assert_true(len(filenames_after))
    ...
    def remove_files_created_during_previous_tests(self):
